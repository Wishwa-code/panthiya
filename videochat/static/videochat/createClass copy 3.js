...
function NextComponent({ onMicToggle, onCamToggle, onHangup, remoteUsers }) {
  return (
    <div className="next-component">
      ...
      <button onClick={onCamToggle}>Toggle Cam</button>
...
function Main({ localMic, localCam, remoteUsers , onMicToggle, onCamToggle, onHangup}) {
  return (
    <>
      <UserList
        localMic={localMic}
    ...
    function CreateClass({ appId, channel, token }) {
        ...
        const handleJoinOrCreate = async () => {
            try {
                ...
                console.log("Joining or creating call with data:", data);
                await joinCall(data);
            } catch (joinErr) {
                try {
                    console.error("Join failed, attempting to create meeting", joinErr);
        ...
        React.useEffect(() => {
            console.log("actaully Cleaning up client");
        ...
        React.useEffect(() => {
            if (isConnected) {
                ...
                console.log('🕶️🕶️🕶️remote users🕶️🕶️🕶️',remoteUsers)
                if (container) {
                    container.innerHTML = "";
                    ReactDOM.render(
                       <Main
                            ...
                            onCamToggle={handleCameraToggle}
            ...
            return () => {
                console.log("Mocking cleaning up");
