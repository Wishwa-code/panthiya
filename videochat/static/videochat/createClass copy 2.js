...
function CreateClass({ appId, channel, token }) {
        ...
        const handleJoinOrCreate = async () => {
          console.log("Joining or creating meeting with appId:", appId, "channel:", channel, "token:", token);
          try {
            // First attempt to join directly
            const data = {
              appId,
            ...
            await joinCall(data); // If this fails, we catch below
          } catch (joinErr) {
            console.warn("Join failed, trying to create a new meeting...", joinErr);

            // If join fails, create a new meeting
            try {
              ...
              await joinCall(data); // Join newly created channel
            } catch (createErr) {
...
document.querySelectorAll('.join-class-button').forEach((el, index) => {
